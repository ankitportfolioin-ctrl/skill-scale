...
export function AccountDashboardModal() {
  ...
  return createPortal(
    ...
    >
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 sm:px-6 sm:py-4 bg-slate-50/70">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className="flex size-9 sm:size-10 shrink-0 items-center justify-center rounded-xl bg-[#312E81] text-white font-bold text-sm sm:text-base uppercase shadow-xs">
              {user.name ? user.name.charAt(0) : 'U'}
            </div>
            <div className="min-w-0">
              <h2 id="account-dashboard-title" className="font-heading text-sm sm:text-lg font-bold text-slate-900 leading-tight truncate">
                {user.name}
              </h2>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2 py-1.5 sm:px-2.5 text-xs font-semibold text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition cursor-pointer"
            >
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-slate-200 bg-white px-3 sm:px-6 overflow-x-auto scrollbar-none">
          {tabs.map((tab) => {
            const Icon = tab.icon
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 sm:gap-2 py-2.5 px-2.5 sm:py-3 sm:px-3 text-xs sm:text-sm font-semibold border-b-2 transition whitespace-nowrap cursor-pointer ${
                  isActive
                    ? 'border-[#312E81] text-[#312E81]'
                    : 'border-transparent text-slate-500 hover:text-slate-900 hover:border-slate-300'
                }`}
              >
                <Icon className={`size-3.5 sm:size-4 ${isActive ? 'text-[#312E81]' : 'text-slate-400'}`} />
                <span>{tab.label}</span>
