...
export function CartDrawer({ open, items, onClose, onIncrease, onDecrease, onRemove, onCheckout, currency }: CartDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-labelledby="cart-title">
      ...
      <section className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col bg-background shadow-2xl animate-in slide-in-from-right duration-300">
                ...
                <li key={product.id} className="flex gap-4 border-b pb-5">
                  <div className="relative size-20 shrink-0 overflow-hidden rounded-xl bg-muted">
                    <Image
                      ...
                      alt=""
                      fill
                      unoptimized={product.image.startsWith('http') || product.image.startsWith('data:') || product.image.startsWith('//')}
                      sizes="80px"
        ...
        {items.length > 0 && (
          <footer className="border-t bg-muted/40 p-5 sm:p-6">
            ...
            <Button type="button" onClick={onCheckout} className="h-12 w-full bg-success text-base text-success-foreground hover:bg-success/90">Demo checkout</Button>
            <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
              <ShieldCheck aria-hidden="true" className="size-3.5 text-success" />
              Checkout is disabled in this integration-free demo.
            </p>
          </footer>
        )}
      </section>
